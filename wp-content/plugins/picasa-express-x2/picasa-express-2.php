...
Plugin URI: http://wott.net.ru/picasa-express
...
if (!class_exists("PicasaExpressX2")) {
	class PicasaExpressX2 {
		...
		function walk_blogs($func) {
			...
			$walk = isset($_GET['sitewide']); // (de)activate by command from site admin
			...
			if ( $walk && function_exists('get_blog_list') && function_exists('switch_to_blog')) {
		...
		function load_textdomain() {
			if ( function_exists('load_plugin_textdomain') ) {
				load_plugin_textdomain('pe2', PLUGINDIR.'/'.dirname(plugin_basename(__FILE__)));
			}
		}

		function add_footer_link() {
			echo "<p class=\"footer-link\" style=\"font-size:75%;text-align:center;\"><a href=\"http://wott.net.ru/\">".__('With Picasa plugin by Wott','pe2')."</a></p>";
		}
...
//			echo "$tag: ".count($m[1])."<br/>";
		...
		function gallery_shortcode($atts, $content) {
			...
			if ($album) {
				...
				if (is_wp_error($rss)) {
					...
					if ($thumb_w && $thumb_h) {
						// both sizes and crop
						if ($thumb_crop && $thumb_w == $thumb_h) $new_thumb_size = '/s'.$thumb_w.'-c';
		...
		function add_settings_page() {
			...
			add_options_page(__('Picasa Express x2', 'pe2'), __('Picasa Express x2', 'pe2'), 8, 'picasa-express-2', array(&$this, 'settings_form'));
		...
		function contextual_help($help, $screen) {
			if ( 'settings_page_picasa-express-2' == $screen ) {
				...
				<div class="metabox-prefs">
					<a href="http://wott.net.ru/picasa-express">$homepage</a>
				</div>
HELP_TEXT;
			}
			return $help;
		...
		function settings_form(){
			...
			<form method="post" action="options.php">
				...
				<a href="http://wott.net.ru/donate/" target="_blank" style="position:relative;display:block;float:right;margin:10px;height: 10px;">
					<img src="http://lh6.ggpht.com/_P3-vavBhxe8/TKAJwocDI3I/AAAAAAAAAhQ/VZ9rmzWqXA4/s128/Paypal_button1.png" alt="PayPal donate" title="Donate to Wott via PayPal to support plugin developing" width="128" height="62" />
				</a>
				<?php } ?>
				...
				<table class="form-table">
					...
					foreach( $editable_roles as $role => $details ) {
						...
						$out .= "<label><input name=\"pe2_roles[$role]\" type=\"checkbox\" value=\"1\" ".checked('1', $option[$role],false)."/>$name</label><br/>";
					}
